...
### TCP扫描
TCP标志：SYN、ACK、RST、FIN
1、TCP会三次握手（SYN、SYN ACK、ACK）
```nmap -sT```: 
```C++
// 发送RST进行会话中断；正常结束TCP会话需要发送FIN；需要整个完整的握手过程花费的时间比较长
// open 开放的
1          SYN          1
2          SYN ACK      2
3          ACK          3
...
``` nmap -Pn ```：相当于"No Ping"，绕过防火墙的阻挡。需要很多时间（nmap会假设目标主机正在运行，如果目标主机没有运行会重复探测）
